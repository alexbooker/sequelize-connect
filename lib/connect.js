...
db.connect = function(database, username, password, options, connectionOptions) {
  ...
  // If the third option is an object
  // then assume they passed in a connectionstring
  ...
  var sequelize         = new db.Sequelize(database, username, password, options)
    ...
    , discoverPaths     = connectionOptions.discover || db.discover
    , models            = {};


  return discover(discoverPaths, db.matcher, logger)
    .each(function(path){
